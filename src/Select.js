...
class Select extends React.Component {
  constructor(props) {
    ...
    const initialValue =
      this.props.model[this.props.form.key] ||
      (this.props.form.titleMap != null
    ...
    const mockEvent = { target: { value: initialValue } };
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.model && nextProps.form.key) {
      this.setState({
        currentValue:
          nextProps.model[nextProps.form.key] ||
          (nextProps.form.titleMap != null
            ? nextProps.form.titleMap[0].value
            : "")
      });
    }
  }
  ...
  render() {
    const menuItems = this.props.form.titleMap.map((item, idx) => (
      <MenuItem key={idx} primaryText={item.name} value={item.value} />
    ));
